...
**Última atualização:** 2026-09-24 · **Entrega atual concluída:** fundação visual App Cli Manager com azul ServiceFlow · **F4-P2 (DRE simples em regime de caixa, ADR-026)** · **F4-P1/P2 concluídas** · **F3 completo (P1–P5)** · **F2 completo (P1–P5)** · **RLS validado em banco real** · **Próxima:** aplicar a fundação visual nas telas de produto e validar o fluxo completo em staging
...
| `flutter analyze` | ✅ **Executado em 2026-09-24 — 0 issues.** Fundação visual adicionada com tokens, tipografia, componentes neomórficos e breakpoints responsivos. |
| Testes Flutter | ✅ **`flutter test` — 319 testes passando em 2026-09-24.** Inclui cobertura da fundação visual, componentes neomórficos, modais arredondados e modos de navegação responsiva. |
...
| Migrations | ✅ **0054–0062 aplicadas (2026-07-28).** 0054–0058 via `supabase db push`; 0059–0062 via SQL Editor (consolidados em `dist/`). ⚠️ **0059–0062 não constam no histórico de migrations do Supabase**: rode `supabase migration repair --status applied 0059 0060 0061 0062` para o `db push` futuro não tentar reaplicá-las. Todas são idempotentes, então reaplicar não quebra. ⚠️ Categorias semeadas pela 0059 podem ter acentos corrompidos (UTF-8 lido como MacRoman na colagem) — corrigir com `dist/corrige_acentos_categorias.sql`. ⏳ Testes de isolamento `0027`–`0032` escritos e ainda não executados. |
