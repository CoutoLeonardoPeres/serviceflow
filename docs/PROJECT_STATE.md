...
| Migrations | ✅ **0054–0061 aplicadas e verificadas no banco (2026-07-28).** 0054–0058 via `supabase db push`; 0059–0061 via SQL Editor (consolidados em `dist/`). Confirmado: 4 tabelas do catálogo, 3 funções do módulo de materiais, 5 gatilhos e 3 funções da 0061. ⚠️ **0059, 0060 e 0061 não constam no histórico de migrations do Supabase** — foram aplicadas fora do `db push`. Todas são idempotentes, mas convém `supabase migration repair --status applied 0059 0060 0061`. ⚠️ As categorias semeadas pela 0059 chegaram com acentos corrompidos (UTF-8 lido como MacRoman na colagem); corrigir com `dist/corrige_acentos_categorias.sql`. ⏳ Testes de isolamento `0027`–`0032` escritos e ainda não executados. |
