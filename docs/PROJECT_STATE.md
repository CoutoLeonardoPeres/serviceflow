...
| Testes SQL | ⏳ 12 de 13 roteiros de `test/isolation/` **reescritos em 2026-07-24** como testes automatizados com verificação real de isolamento entre tenants (antes, `0007`–`0013` eram roteiros manuais comentados sem asserção). `0006` (link público) permanece manual — depende de orçamento aprovado real. Nenhum teste foi executado ainda (sandbox sem PostgreSQL/root). Runbook completo em `docs/FASE1_RUNBOOK_SEGURANCA_RLS.md`, com `supabase/seed/dev_seed.sql` atualizado e 3 scripts novos (`create_test_users.sh`, `apply_test_uuids.sh`, `run_isolation_tests.sh`) para automatizar a execução — **critério bloqueador do aceite do MVP** |
