...
| Migrations pendentes | ⏳ **0054–0060 escritas e não aplicadas** (2026-07-28). Nesta ordem: `0054` itens no link público; `0055` decisão interna do orçamento; `0056` OS automática na aprovação; `0057` máquina de estados da OS, guarda de cobrança e `my_permissions()`; `0058` encerramento da cadeia e reabertura em 30 dias; `0059` catálogo de materiais, fornecedor completo e preços (**rollback perde dados**); `0060` importação de preços por planilha. `0057` e `0058` dependem de `0056`; `0060` depende de `0059`. Testes de isolamento `0027`–`0031` escritos e ainda não executados. Enquanto `0057` não for aplicada, `hasPermissionProvider` cai no modo permissivo (mostra ações que o banco recusa) — é intencional. |
